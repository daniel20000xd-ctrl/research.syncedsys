...
def get_client():
    ...
    if _client is None:
        ...
        _client = boto3.client(
            ...
            config=Config(retries={"max_attempts": 3, "mode": "standard"}, max_pool_connections=32),
        )
    return _client
...
def upload_text(key: str, text: str) -> str:
    """Upload UTF-8 text to R2 under `key`; return the key."""
...
def list_keys(prefix: str):
    ...
    for page in get_client().get_paginator("list_objects_v2").paginate(Bucket=R2_BUCKET, Prefix=prefix):
